...
class Event_independence_fightSounEunju(GameActivity: GameActivity, eventName: String, weight: Int, isAvailable: Boolean)
    ...
    override fun eventEffect(tmp: Int) {
        if(tmp == 1) { // 소운이를 고를경우
            ...
            val randomValue = Math.random()
            setPreScript(getScript("event_independence_fightSounEunju_tmp"))
            if(randomValue < 0.5){
                loseRandomItem()
            }
            else{
